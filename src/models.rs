...
impl PhoneHomeData {
    ...
    pub fn extract_field_value(&self, field_name: &str) -> Option<String> {
        match field_name {
            ...
            "public_keys" => {
                self.public_keys.as_ref().map(|keys| keys.join(","))
            },
            "local_ipv4" => {
            ...
            _ => {
                ...
                self.additional_data.get(field_name)
                    .and_then(|value| {
                        match value {
    ...
    pub fn process(&self, config: &crate::config::PhoneHomeConfig) -> ProcessedPhoneHomeData {
        ...
        for field_name in &config.fields_to_extract {
            let value = self.extract_field_value(field_name)
...
impl Default for PhoneHomeData {
    fn default() -> Self {
        Self {
            instance_id: None,
            public_keys: None,
...
mod tests {
    ...
    fn test_field_extraction() {
        ...
        assert_eq!(
            phone_home_data.extract_field_value("nonexistent"),
            None
        );
    }
